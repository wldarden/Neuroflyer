...
void SkirmishConfigScreen::on_draw(AppState& /*state*/, Renderer& /*renderer*/,
    ...
    ui::slider_float("World Width", &config_.world.world_width, 1000.0f, 10000.0f);
    ui::slider_float("World Height", &config_.world.world_height, 1000.0f, 10000.0f);

    int squads = static_cast<int>(config_.world.num_squads);
    if (ui::input_int("Squads per Team", &squads, 1, 4)) {
        config_.world.num_squads = static_cast<std::size_t>(squads);
    }

    int fighters = static_cast<int>(config_.world.fighters_per_squad);
    if (ui::input_int("Fighters per Squad", &fighters, 2, 20)) {
        config_.world.fighters_per_squad = static_cast<std::size_t>(fighters);
    }

    int towers = static_cast<int>(config_.world.tower_count);
    if (ui::input_int("Towers", &towers, 0, 200)) {
        config_.world.tower_count = static_cast<std::size_t>(towers);
    }

    int tokens = static_cast<int>(config_.world.token_count);
    if (ui::input_int("Tokens", &tokens, 0, 200)) {
        config_.world.token_count = static_cast<std::size_t>(tokens);
    }
    ...
    ui::slider_float("Base HP", &config_.world.base_hp, 100.0f, 10000.0f);
    ui::slider_float("Base Radius", &config_.world.base_radius, 20.0f, 300.0f);
    ui::slider_float("Base Bullet Damage", &config_.world.base_bullet_damage, 1.0f, 100.0f);
    ImGui::Spacing();

    // --- Physics ---
    ui::section_header("Physics");

    ui::slider_float("Rotation Speed", &config_.world.rotation_speed, 0.01f, 0.2f);
    ui::slider_float("Bullet Max Range", &config_.world.bullet_max_range, 200.0f, 3000.0f);
    ui::checkbox("Wrap N/S", &config_.world.wrap_ns);
    ui::checkbox("Wrap E/W", &config_.world.wrap_ew);
    ui::checkbox("Friendly Fire", &config_.world.friendly_fire);
    ImGui::Spacing();
