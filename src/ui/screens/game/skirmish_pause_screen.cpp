...
SkirmishPauseScreen::SkirmishPauseScreen(
...
void SkirmishPauseScreen::on_draw(AppState& state, Renderer& renderer,
                                   UIManager& ui) {
    ...
    ImGui::SetNextWindowSize(ImVec2(screen_w * 0.8f, screen_h * 0.9f),
                             ImGuiCond_Always);
    ImGui::Begin("Squad Skirmish — Paused", nullptr,
        ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove |
        ImGuiWindowFlags_NoCollapse);

    // Tab bar
    if (ImGui::BeginTabBar("##SkirmishPauseTabs")) {
    ...
    if (active_tab_ == Tab::Evolution) {
        ...
        for (std::size_t rank = 0; rank < sorted_indices_.size(); ++rank) {
            ...
            char label[64];
            if (is_elite) {
                std::snprintf(label, sizeof(label), "Squad #%zu (Elite)",
                    rank + 1);
            } else {
            ...
            ui.push_modal(std::make_unique<InputModal>(
                ...
                [this, &state](const std::string& base_name) {
                    ...
                    for (std::size_t rank = 0; rank < sorted_indices_.size(); ++rank) {
                        ...
                        const auto& team = population_[pop_idx];
                ...
                "skirmish-g" + std::to_string(generation_)));
        }
