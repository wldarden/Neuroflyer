...
const char* phase_name(AttackRunPhase phase) {
    switch (phase) {
    case AttackRunPhase::Phase1: return "Attack 1";
    case AttackRunPhase::Phase2: return "Attack 2";
    case AttackRunPhase::Phase3: return "Attack 3";
    case AttackRunPhase::Done:   return "Done";
    }
    return "?";
}
...
AttackRunScreen::AttackRunScreen(Snapshot source_snapshot,
...
AttackRunScreen::~AttackRunScreen() {
    destroy_net_viewer_view(net_viewer_state_);
}

// ==================== Initialization ====================

void AttackRunScreen::initialize(AppState& state) {
    ...
    population_ = create_population_from_snapshot(
        fighter_snap, config_.population_size, evo_config_, rng_);
    ...
    std::cout << "[AttackRuns] Initialized with " << population_.size()
              << " fighters from variant '" << variant_name_ << "'\n";
}

// ==================== Input handling ====================

bool AttackRunScreen::handle_input(UIManager& ui) {
...
void AttackRunScreen::evolve_generation(AppState& /*state*/) {
    ...
    std::cout << "[AttackRuns Gen " << generation_ << "] Best: "
    ...
    // Sync drill_ship_teams_ to match new population size
    drill_ship_teams_.assign(population_.size(), 0);

    // Sync config population size with evolved population
    config_.population_size = population_.size();
...
void AttackRunScreen::render_world(Renderer& renderer) {
    if (!session_) return;
...
void AttackRunScreen::render_hud(Renderer& renderer) {
    int game_panel_w = renderer.game_w();
    int game_panel_h = renderer.screen_h();
    ...
    if (show_net_viewer) {
        ...
        ImGui::SetNextWindowSize(ImVec2(static_cast<float>(info_w), 100.0f),
                                 ImGuiCond_Always);
        ImGui::Begin("##AttackRunHUD", nullptr,
            ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize |
            ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse |
            ImGuiWindowFlags_NoScrollbar);

        ImGui::TextColored(ImVec4(0.3f, 0.8f, 1.0f, 1.0f), "ATTACK RUNS");
        ...
        ImGui::SetNextWindowSize(
            ...
            ImGuiCond_Always);
        ImGui::Begin("##AttackRunInfo", nullptr,
            ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize |
            ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse);
...
void AttackRunScreen::on_draw(AppState& state, Renderer& renderer,
                               UIManager& ui) {
    if (!initialized_) {
        initialize(state);
    }
...
void AttackRunScreen::post_render(SDL_Renderer* sdl_renderer) {
